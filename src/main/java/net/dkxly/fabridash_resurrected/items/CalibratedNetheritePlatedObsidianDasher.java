...
public class CalibratedNetheritePlatedObsidianDasher extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (user.isSneaking()) {
            ...
            if (!world.isClient) {
                try {
                    FabridashResurrected.dash(user, 5 * world.getGameRules().getInt(FabridashResurrectedMod.DASH_MULTIPLIER), stack.get(DataComponentTypes.CUSTOM_MODEL_DATA).value());
                } catch (Exception ignored) {}
            }
